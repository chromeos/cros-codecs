...
pub trait StatelessH264DecoderBackend: StatelessDecoderBackend<Rc<Sps>> {
...
pub struct H264DecoderState<B: StatelessDecoderBackend<Rc<Sps>>> {
...
impl StatelessCodec for H264 {
    ...
    type DecoderState<B: StatelessDecoderBackend<Rc<Sps>>> = H264DecoderState<B>;
}
    ...
    B::Handle: Clone,
{
    ...
    fn check_first_mb_in_slice(&mut self, cur_pic: &mut CurrentPicState<B>, slice: &Slice<&[u8]>) {
            ...
            CurrentMacroblockTracking::SeparateColorPlane(current_macroblock) => {
                match current_macroblock.entry(slice.header().colour_plane_id) {
                    ...
                    Entry::Occupied(mut current_macroblock) => {
                        ...
                        assert!(slice.header().first_mb_in_slice >= *current_macroblock);
